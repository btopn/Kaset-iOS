...
///
/// Ports Kaset's macOS `ImageCache` to iOS: identical logic (memory + disk
/// cache, ImageIO downsampling, in-flight coalescing, LRU eviction, memory
/// pressure response) with `NSImage` swapped for `UIImage`.
actor ImageCache {
    ...
    func image(for url: URL, targetSize: CGSize? = nil) async -> UIImage? {
    ...
    /// Creates a UIImage from data, optionally downsampling for memory efficiency.
    private static func createImage(from data: Data, targetSize: CGSize?) -> UIImage? {
    ...
    nonisolated private func loadFromDisk(url: URL, targetSize: CGSize? = nil) -> UIImage? {
