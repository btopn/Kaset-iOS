...
///
/// Ports Kaset's macOS `ColorExtractor` to iOS: identical k-means-style
/// averaging logic, with `NSImage`/`NSColor` swapped for `UIImage`/`UIColor`.
enum ColorExtractor {
    ...
    struct ColorPalette: Equatable {
        ...
        static let `default` = ColorPalette(
            primary: Color(uiColor: UIColor(white: 0.15, alpha: 1)),
            secondary: Color(uiColor: UIColor(white: 0.08, alpha: 1)),
            lightTint: Color(uiColor: UIColor.systemBlue).opacity(0.3)
        )
    }

    /// Extracts a color palette from a UIImage.
    /// Uses weighted averaging on a downsampled image for performance.
    /// - Parameter image: The source image.
    /// - Returns: A ColorPalette with primary and secondary colors.
    static func extractPalette(from image: UIImage) -> ColorPalette {
        ...
        return ColorPalette(
            primary: Color(uiColor: primary),
            secondary: Color(uiColor: secondary),
            lightTint: Color(uiColor: lightTint)
        )
    }
    ...
    static func extractPalette(from data: Data) async -> ColorPalette {
        ...
        await Task(priority: .userInitiated) {
            guard let image = UIImage(data: data) else {
                return ColorExtractor.ColorPalette.default
            }
