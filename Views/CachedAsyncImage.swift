...
///
/// Ports Kaset's macOS `CachedAsyncImage` to iOS: identical behavior, with
/// `NSImage` swapped for `UIImage` and `Image(nsImage:)` for `Image(uiImage:)`.
struct CachedAsyncImage<Content: View, Placeholder: View>: View {
    ...
    var body: some View {
        ZStack {
            if let image {
                self.content(Image(uiImage: image))
...
/// A simple ProgressView wrapper with proper sizing.
