...
/// Shared context menu items for sharing content via the native iOS share sheet.
///
/// On macOS Kaset drove an `NSSharingServicePicker` from each menu item. On iOS
/// we return a SwiftUI `ShareLink` (or its `Menu`-compatible form) for each
/// shareable type — a single implementation reused everywhere a share affordance
/// appears. Call sites embed these in their `.contextMenu { }` builders.
@MainActor
